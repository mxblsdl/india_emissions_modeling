...
emissionsModel <- function(census,
  ...
  if(lighting_scenario == 1) {
    census <-
      census %>%
      mutate(
        mainlight_electricity = mainlight_electricity + (rowSums(select(., fuelFracVar)) * eff * !!as.name(buf_dist)),
        mainlight_kerosene = mainlight_kerosene  - (mainlight_kerosene * eff * !!as.name(buf_dist)),
        mainlight_otheroil = mainlight_otheroil - (mainlight_otheroil * eff * !!as.name(buf_dist)),
        mainlight_other = mainlight_other - (mainlight_other * eff * !!as.name(buf_dist)))
  }
  
    # Drop electricity from emissions calculations
    fuelFracVar <-  str_subset(fuelFracVar, "electricity", negate = T)
    
    # Sum all lighting
    census %<>% 
      mutate(allLighting = rowSums(select(., fuelFracVar)))
    ...
    for (month in months) {
      ...
      census <- cbind(census, energyToFuel_sh_v3(df = census, monthNum = month))
    }
    
    # Runs the urban space heating function everytime except baseline
    # if (baseline != 1) {
    #   census <- urbanSpace(census)
    # }
    ...
    if (space_scenario == 1) {
      ...
      # calc new columns
      census %<>% 
        mutate(sh_lpg_kg_1 = ((!!as.name(sh_cols[[1]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_2 = ((!!as.name(sh_cols[[2]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_3 = ((!!as.name(sh_cols[[3]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_4 = ((!!as.name(sh_cols[[4]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_5 = ((!!as.name(sh_cols[[5]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_6 = ((!!as.name(sh_cols[[6]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_7 = ((!!as.name(sh_cols[[7]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_8 = ((!!as.name(sh_cols[[8]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_9 = ((!!as.name(sh_cols[[9]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_10 = ((!!as.name(sh_cols[[10]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_11 = ((!!as.name(sh_cols[[11]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               sh_lpg_kg_12 = ((!!as.name(sh_cols[[12]]) * eff * !!as.name(buf_dist)) * wood$LHV * wood$eff_fuelLevel) / (lpg$LHV * lpg$eff_fuelLevel),
               
               sh_wood_kg_1 = sh_wood_kg_1 - (!!as.name(sh_cols[[1]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_2 = sh_wood_kg_2 - (!!as.name(sh_cols[[2]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_3 = sh_wood_kg_3 - (!!as.name(sh_cols[[3]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_4 = sh_wood_kg_4 - (!!as.name(sh_cols[[4]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_5 = sh_wood_kg_5 - (!!as.name(sh_cols[[5]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_6 = sh_wood_kg_6 - (!!as.name(sh_cols[[6]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_7 = sh_wood_kg_7 - (!!as.name(sh_cols[[7]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_8 = sh_wood_kg_8 - (!!as.name(sh_cols[[8]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_9 = sh_wood_kg_9 - (!!as.name(sh_cols[[9]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_10 = sh_wood_kg_10 - (!!as.name(sh_cols[[10]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_11 = sh_wood_kg_11 - (!!as.name(sh_cols[[11]]) * eff * !!as.name(buf_dist)),
               sh_wood_kg_12 = sh_wood_kg_12 - (!!as.name(sh_cols[[12]]) * eff * !!as.name(buf_dist)))
    }
